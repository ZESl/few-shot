...
class OmniglotDataset(Dataset):
    def __init__(self, subset):
        ...
        # print(self.df.iloc[[1]].to_string())
        # Create dicts
...
class MotionDataset(Dataset):
    def __init__(self, subset):
        """Motion dataset for debugging/testing purposes
        ...
        if subset == 'background':
            filename = 'Motion/Data_motion_test.csv'
        elif subset == 'evaluation':
            filename = 'Motion/Data_motion_val.csv'
        else:
            raise (ValueError, 'subset must be one of (background, evaluation)')
    ...
    def __getitem__(self, item):
        tmp = self.df.iloc[[item]].drop(['class_id', 'id'], axis=1)
        features = np.array(tmp, dtype=np.float)
